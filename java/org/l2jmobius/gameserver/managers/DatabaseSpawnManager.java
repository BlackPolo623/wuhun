...
public class DatabaseSpawnManager
{
	...
	public void addNewSpawn(Spawn spawn, long respawnTime, double currentHP, double currentMP, boolean storeInDb)
	{
		...
		{
			try (Connection con = DatabaseFactory.getConnection();
				PreparedStatement statement = con.prepareStatement("INSERT INTO npc_respawns (id, x, y, z, heading, respawnTime, currentHp, currentMp, Name) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)"))
			{
				...
				statement.setString(9, spawn.getName());
				statement.execute();
			}
			catch (Exception e)
			{
	...
	public Npc addNewSpawn(Spawn spawn, boolean storeInDb)
	{
		...
		if (storeInDb)
		{
				...
				PreparedStatement statement = con.prepareStatement("INSERT INTO npc_respawns (id, x, y, z, heading, respawnTime, currentHp, currentMp, Name) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)"))
			{
				...
				statement.setString(9, npc.getName());
				statement.execute();
			}
			catch (Exception e)
			{
