...
public class PetHatchingSystem extends Script
{
	...
	private String buildSlotHtml(int slotIndex, PetHatchData data)
	{
		...
		sb.append("<tr>");
		sb.append("<td width=80>槽位 ").append(slotIndex + 1).append("</td>");

		if (data == null)
		{
			sb.append("<td width=200>空閒</td>");
			sb.append("<td width=120><button value=\"開始孵化\" action=\"bypass -h Quest PetHatchingSystem start_hatch ").append(slotIndex).append("\" width=100 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
		}
		else
		{
			...
			if (remainingTime <= 0)
			{
				sb.append("<td width=200>").append(getTierName(data.eggTier)).append("蛋 - 已完成</td>");
				sb.append("<td width=120>等待處理中...</td>");
			}
			else
			{
				sb.append("<td width=200>").append(getTierName(data.eggTier)).append("蛋 - 剩餘").append(remainingTime / 60000).append("分鐘</td>");
				sb.append("<td width=120>進階率: ").append(data.upgradeChance).append("%</td>");
			}
		}
		sb.append("</tr>");
		return sb.toString();
	...
	private String handleCollectionMenu(Player player, int page)
	{
		...
		String[] tierNames = {"一般", "特殊", "稀有", "史詩", "傳說"};
		for (int i = 0; i < 5; i++)
		{
			if (i == page)
				pageButtons.append("<font color=\"LEVEL\">").append(tierNames[i]).append("</font> ");
			else
				pageButtons.append("<a action=\"bypass -h Quest PetHatchingSystem collection_page_").append(i).append("\">").append(tierNames[i]).append("</a> ");
		}
	...
	private String buildCollectionSlotHtml(int index, int petItemId, boolean stored, boolean hasInInventory)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("<tr>");
		sb.append("<td width=50>").append(index).append("</td>");
		sb.append("<td width=100>ID:").append(petItemId).append("</td>");
		if (stored)
		{
			sb.append("<td width=80><font color=\"00FF00\">已收藏</font></td>");
			sb.append("<td width=120><button value=\"提取\" action=\"bypass -h Quest PetHatchingSystem retrieve_pet ").append(petItemId).append("\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
		}
		else if (hasInInventory)
		{
			sb.append("<td width=80><font color=\"FFFF00\">可收藏</font></td>");
			sb.append("<td width=120><button value=\"收藏\" action=\"bypass -h Quest PetHatchingSystem store_pet ").append(petItemId).append("\" width=80 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
		}
		else
		{
			sb.append("<td width=80><font color=\"FF0000\">未收藏</font></td>");
	...
	private void processHatchCompletion(int playerId, int slotIndex, int eggTier)
	{
		...
		if (maxIncrease > 0 && storedFeed > 0)
		{
			...
			PetHatchingDAO.updateHatchUpgradeChance(playerId, slotIndex, currentChance, data.feedConsumed + feedToUse);
		}
		...
		if (eggTier < 4 && random.nextInt(100) + 1 <= currentChance)
		{
			...
			PetHatchingDAO.updateHatchDataForUpgrade(playerId, slotIndex, getEggIdByTier(newTier), newTier, System.currentTimeMillis(), newHatchTime, BASE_UPGRADE_CHANCE);
			startHatchingTask(playerId, slotIndex, newTier, newHatchTime);
		}
		else
		{
			sendPetByMail(playerId, getRandomPetItem(eggTier), eggTier);
