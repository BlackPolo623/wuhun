...
public class SummonItems extends ItemSkillsTemplate
{
	...
	public boolean onItemUse(Playable playable, Item item, boolean forceUse)
	{
		...
		// ── 魂契系統：區域限制 ──────────────────────────────────────
		final boolean hasSnapshot = PetSnapshotData.getInstance().hasSnapshot(player.getObjectId());
		final boolean inPeaceZone = player.isInsideZone(ZoneId.PEACE);
		final boolean inAllowedInstance = player.isInInstance() &&
			(player.getInstanceWorld().getTemplateId() == 900 || player.getInstanceWorld().getTemplateId() == 802);


		if (hasSnapshot)
		{
			// 有魂契：任何地方都不允許召喚
			player.sendMessage("已締結魂契時，無法直接召喚寵物。");
			player.sendMessage("寵物能力已共享至您的身上，無需召喚。");
			return false;
		}
		else if (!inPeaceZone && !inAllowedInstance)
		{
			// 無魂契：只允許和平區域、玩家基地(900)、寵物農場(802)
			player.sendMessage("野外地區無法召喚寵物。");
			player.sendMessage("請至【玩家基地】或【寵物農場副本】練等，或至孵化NPC締結【魂契】。");
			return false;
		}
		// ────────────────────────────────────────────────────────────
